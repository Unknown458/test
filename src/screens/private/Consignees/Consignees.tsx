...
const Consignees = memo(() => {
	...
	const [consignees, _setConsignees] = useState<PartyInterface[]>([]);
	...
	const [loadingSubConsignees, setLoadingSubConsignees] = useState(true);
	...
	const [subConsignees, setSubConsignees] = useState<PartyInterface[]>([]);
	...
	const initialFetch = useCallback(async () => {
		try {
			...
			if (
				...
				allActiveDeliveryBranches.length &&
				goodsTypeData.length &&
				statesData.length
			) {
				_setConsignees(consigneesData);
				_setBillTypes(billTypesData);
				_setUser(userData);
				_setPaymentTypes(paymentTypeData);
				_setActiveDeliveryBranches(allActiveDeliveryBranches);
	...
	const validateNumbers = useCallback(
		(numbers: string[], lengthCheck = 10) => {
			const errorsArray = numbers.map((number) => {
				if (!number) return 'Number is required.';
				if (
					number.length < lengthCheck ||
					number.length > lengthCheck + 1
				)
	...
	const validateConsignee = useCallback((): boolean => {
		...
		if (!formData.partyName)
			errors.partyName = 'Consignee Name is required.';
		...
		if (!formData.biltyCharge)
			errors.biltyCharge = 'LR Charge is required.';
	...
	const handleSearch = useCallback(
		async (keyword: string) => {
			const array = await getAllConsigneesByCompany();
			if (!keyword || keyword.trim() === '') {
				_setConsignees(array);
				return;
			}
			const regex = new RegExp(keyword.trim(), 'i');
			const filteredConsignees = array.filter((consignee) =>
				regex.test(consignee.partyName)
			);
			_setConsignees(addIndex.addIndex3(filteredConsignees));
		},
		[getAllConsigneesByCompany]
	);

	const handleCreateConsignee = useCallback(async () => {
		...
		try {
			...
			if (
				...
				if (response.data.status === 200) {
					const newConsignee: PartyInterface = {
						...formData,
						partyId: response.data.data,
					};
					_setConsignees((prev) => addIndex.addIndex3([newConsignee, ...prev]));
					handleCloseFormDialog();
					handleOpenAlertDialog('success', 'Created new Consignee.');
				} else {
	...
	const handleUpdateConsignee = useCallback(async () => {
		if (!validateConsignee()) return;
		...
		try {
			...
			if (
				...
				if (response.data.status === 200) {
					const updatedConsignees = consignees.map((obj) =>
						obj.partyId === data.partyId ? { ...data } : obj
					);
					_setConsignees(addIndex.addIndex3(updatedConsignees));
					handleCloseFormDialog();
					handleOpenAlertDialog('success', 'Updated Consignee.');
				} else {
					handleOpenAlertDialog('warning', response.data.data);
				}
			} else {
	...
	const handleDeleteConsignee = useCallback(
		async (data: PartyInterface) => {
			const confirm = window.confirm(
				`Are you sure you want to delete Consignee '${data.partyName}'?`
			);
			if (!confirm) return;
			...
			try {
				...
				if (
					...
					if (response.data.status === 200) {
						...
						_setConsignees(addIndex.addIndex3(updatedConsignees));
						handleOpenAlertDialog('success', 'Deleted Consignee.');
					} else {
						handleOpenAlertDialog('warning', response.data.data);
					}
				} else {
		...
		[consignees, handleLogout, handleOpenAlertDialog, _setConsignees]
	);
	...
	const validateSubPartyForm = () => {
		if (!subPartyForm.subPartyId) {
			setSubPartyFormError((prev) => ({
				...prev,
				subPartyId: 'Consignee is required.',
			}));
		} else {
			...
			if (
				...
				if (response.data.status === 200) {
					for (const consignee of consignees) {
						if (subPartyForm.subPartyId === consignee.partyId) {
							setSubConsignees((prev) =>
								addIndex.addIndex3([consignee, ...prev])
							);
						}
					}
	...
	const handleDeleteSubParty = async (subConsignee: PartyInterface & any) => {
		const confirm = window.confirm(
			`Are you sure you want to delete Sub-Consignee '${subConsignee.partyName}'?`
		);

		if (confirm) {
			const subPartyDetailId = subConsignee.subPartyDetailId as number;
			...
			if (
				...
				if (response.data.status === 200) {
					let subConsigneesCopy = subConsignees.filter(
						(obj) => obj.partyId !== subConsignee.partyId
					);

					setSubConsignees(addIndex.addIndex3(subConsigneesCopy));
					handleOpenAlertDialog('success', 'Deleted Sub-Consignee.');
				} else {
					handleOpenAlertDialog('warning', response.data.data);
				}
			} else {
	...
	const handleSubPartyFormOpen = async (party: PartyInterface) => {
		...
		const subParties = await handleGetSubPartyDetails(party, consignees);

		if (subParties) {
			setSubConsignees(addIndex.addIndex3(subParties));
		}
	};
	...
	const columnsWithBillType = useMemo<MRT_ColumnDef<PartyInterface>[]>(
					...
					<>
						<div className='table-index'>
							{row.original.isActive ? (
								<Tooltip title='Active Consignee'>
									<Chip
										icon={<Done />}
										color='success'
									/>
								...
								<Tooltip title='Disabled Consignee'>
									<Chip
										icon={<Clear />}
										color='error'
									/>
					...
					<>
						{loadingDownload !== row.original.partyId ? (
							<Tooltip title='Download Quotations'>
								<IconButton
									onClick={() =>
										handlePDFdownload(row.original)
									}
								>
									<DownloadOutlined />
								</IconButton>
						...
						{loadingPrint !== row.original.partyId ? (
							<Tooltip title='Print Quotations'>
								<IconButton
									onClick={() => handlePDFprint(row.original)}
								>
									<PrintOutlined />
								</IconButton>
							</Tooltip>
						...
						<Tooltip title='Sub-Consignees'>
							<IconButton
								onClick={() =>
									handleSubPartyFormOpen(row.original)
								}
							>
								<ListAltOutlined />
							</IconButton>
						</Tooltip>
						<Tooltip title='Delete Consignee'>
							<IconButton
								onClick={() =>
									handleDeleteConsignee(row.original)
								}
							>
								<DeleteOutline />
							</IconButton>
						</Tooltip>
						<Tooltip title='Edit Consignee'>
							<IconButton
								onClick={() =>
		...
		[handleDeleteConsignee, handleOpenFormDialog, billTypes]
	);
				...
				Cell: ({ row }) => {
					return (
						<>
							<div className='table-index'>
								{row.original.isActive ? (
									<Tooltip title='Active Sub-Consignee'>
										<Chip
											icon={<Done />}
											color='success'
										/>
									...
									<Tooltip title='Disabled Sub-Consignee'>
										<Chip
											icon={<Clear />}
											color='error'
										/>
					...
					return (
						<>
							<Tooltip title='Delete'>
								<IconButton
									onClick={() =>
										handleDeleteSubParty(row.original)
									}
								>
									<DeleteOutline />
								</IconButton>
				...
				>
					...
					>
						...
						>
							...
							>
								<InputLabel htmlFor='bilty-charges'>
									LR Charge
								</InputLabel>
								<OutlinedInput
									label='LR Charge'
			...
			>
				...
				>
					...
					>
						...
						>
							<InputLabel>Consignees</InputLabel>
							<Select
								label='Consignees'
								value={
									subPartyForm.subPartyId
										? subPartyForm?.subPartyId
										: ''
								}
								onChange={(e) => {
					...
					<div
						...
						className='table-container'
					>
						{loadingSubConsignees ? (
							<Fallback state='loading' />
						) : (
							<>
								{' '}
								{!loadingSubConsignees &&
								subConsignees.length === 0 ? (
